...
const TURTLE = (function () {
	...
	class Turtle {
		...
		// （ライブラリ内だけで使用）前に進む（歩数）
		_goPrep(step) {
			const limit = this._checkAniRemain();
			if (limit === 0) return this;
			const pc = this._doGo(step, limit);
			this._checkAniEnd(pc);
			return this;
		}

		// （ライブラリ内だけで使用）実際に進む
		_doGo(step, limit) {
			const x = this._x, y = this._y, dir_ = this._dir - 90, s = this._step, d = step * s;
			const pc = this._liner.line(x, y, dir_, d, limit, this._area);

			if (this._visible) {
				const r = rad(dir_);
				const x1 = x + d * Math.cos(r);
				const y1 = y + d * Math.sin(r);
				this._curAs = [{ x0: x, y0: y }, { x: x1, y: y1 }];
			}
			return pc;
		}
		...
		_turnPrep(deg) {
			const limit = this._checkAniRemain();
			if (limit === 0) return this;
			const pc = this._doTurn(deg, limit);
			this._checkAniEnd(pc);
			return this;
		}
		...
		_doTurn(deg, limit) {
			...
			if (this._visible) {
				const bx = this._x, by = this._y, dir_ = this._dir - 90;
		...
		direction(deg) {
			...
			if (this._checkAniRemain() === 0) return this;
		...
		curveLeft(step0, deg, step1, opt_deg, opt_step) {
			...
			if (opt_deg === undefined) {
				return this._curvePrep(step0, -deg, step1);
			} else {
				return this._curvePrep(step0, -deg, step1, -opt_deg, opt_step);
			}
		}
		...
		_curvePrep(step0, deg, step1, opt_deg, opt_step) {
			const limit = this._checkAniRemain();
			if (limit === 0) return this;
			const pc = this._doCurve(step0, deg, step1, opt_deg, opt_step, limit);
			this._checkAniEnd(pc);
			return this;
		}
		...
		_doCurve(step0, deg, step1, opt_deg, opt_step, limit) {
			...
			const d0 = step0 * s, d1 = step1 * s, d2 = opt_step * s;

			let pc;
			if (opt_deg === undefined) {
				pc = this._liner.quadCurve(x, y, dir_, d0, deg, d1, limit, this._area);
			} else {
				pc = this._liner.bezierCurve(x, y, dir_, d0, deg, d1, opt_deg, d2, limit, this._area);
			}
			if (this._visible) {
				const r0 = rad(dir_), r1 = rad(dir_ + deg);
		...
		_arcPrep(r, deg, isLeft) {
			const limit = this._checkAniRemain();
			if (limit === 0) return;
			const pc = this._doArc(r, deg, isLeft, limit);
			this._checkAniEnd(pc);
			return this;
		}
		...
		_doArc(r, deg, isLeft, limit) {
			...
			const pc = this._liner.arc(lsp, ltp, rot * 180.0 / Math.PI, p.w, p.h, p.deg0, p.deg1, isLeft, limit, this._area);

			if (this._visible) {
				this._curAs = [{ cx: lsp, cy: ltp, w: p.w, h: p.h, r: rot }];
			}
			return pc;
		}
		...
		_checkAniEnd(powerConsumption) {
			if (!this._visible) return;  // アニメーション表示でなかったら何もしない

			this._aniRemain -= powerConsumption;
			if (this._aniRemain <= 0) {
				const p = this._pen;
				this.penUp();
				...
				this._drawTurtle(p);
				if (this.onPenChange !== null) {
					if (this._lastPenState !== p) this.onPenChange(this, p);
				}
				if (this.onMove !== null) {
					this.onMove(this, this._x, this._y, p);
				}
		...
		_drawTurtle(pen) {
			...
			// ホームの場所が変えられていたら、ホームの三角をかく
			if (this._homeX !== 0 || this._homeY !== 0 || this._homeDir !== 0) {
				c.save();
				c.translate(this._homeX, this._homeY);
				c.rotate(rad(this._homeDir));
				this._drawTriangle(c, true, 'Purple', '', 'Magenta');
				c.restore();
			}
			c.save();
			this.localize();
			this._drawTriangle(c, pen, 'SeaGreen', 'DarkSeaGreen', 'Lime');
			c.restore();

			c.save();
			this._drawFunction(c, this._curFn);
			c.restore();
		...
		_drawTriangle(c, pen, downColor, upColor, lineColor) {
			c.beginPath();
			c.moveTo(0, -10);
			c.lineTo(8, 8);
			c.lineTo(-8, 8);
		...
		_drawFunction(c, curFn) {
			const offX = this._x <= 0 ? 48 : -48;
			const offY = this._y <= 0 ? 48 : -48;
			this._curFnPosX = this._curFnPosX * 0.95 + (this._x + offX) * 0.05;
			this._curFnPosY = this._curFnPosY * 0.95 + (this._y + offY) * 0.05;
			...
			c.textAlign = 'center';
			c.translate(this._curFnPosX, this._curFnPosY);
			setShadow(c, 4, 2);
		...
		circle(r, deg = 360, anticlockwise = false) {
			...
			this._drawShape((limit) => {
				if (this._visible) {
					this._curAs = [{ cx: cx, cy: cy, w: p.w, h: p.h, r: dr }];
				}
		...
		_drawShape(fn, opt_x, opt_y) {
			...
			this.save();
			if (pen) this.penUp();
			if (opt_x !== undefined && opt_y !== undefined) this.moveTo(opt_x, opt_y);
			if (pen) this.penDown();
			const pc = fn(limit);
			this._checkAniEnd(pc);
			if (pen) this.penUp();
		...
		// （ライブラリ内だけで使用）実際に絵をかく
		_drawActually() {
			if (this._curMode.match(/(fill|stroke|clip|none)/)) {
				const ms = this._curMode.replace(/(fill|stroke|clip)/g, '$1,').replace(/,$/, '').split(',');
				for (let m of ms) {
					switch (m) {
					case 'fill':
					...
					case 'stroke':
						...
						break;
					case 'clip':
						if (this._aniFinished) this._ctx.clip();
						break;
					}
				}
			} else {
				for (let m of this._curMode) {
					switch (m) {
					case 'f':
						this._fill.draw(this._ctx, this._area);
						break;
